...
public class ResponseResultBodyAdvice implements ResponseBodyAdvice<Object> {
    ...
    public Object beforeBodyWrite(Object body, MethodParameter returnType, MediaType selectedContentType, Class<? extends HttpMessageConverter<?>> selectedConverterType, ServerHttpRequest request, ServerHttpResponse response) {
        ...
        // 排除不需要包装的类型：Result（已包装）、TableDataInfo（分页数据）、File（文件）、ResponseEntity（响应实体）
        if (body instanceof Result || body instanceof io.github.yangyouwang.common.base.domain.TableDataInfo || body instanceof File || body instanceof ResponseEntity) {
            return body;
        }
